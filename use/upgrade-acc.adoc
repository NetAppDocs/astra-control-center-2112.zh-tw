...
NOTE: 您必須已經擁有 <<Set up acc-helm-repo to upgrade Traefik and Cert-manager,設定acc helm-repo>> 完成下列程序之前。
...
NOTE: 您必須已完成 <<Update Traefik service using acc-helm-repo,Traefik更新>> 和 <<Set up acc-helm-repo to upgrade Traefik and Cert-manager,在Helm中新增了acc hel-repo>> 完成下列程序之前。
